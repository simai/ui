...
- Канонические утилиты `justify-*`, `gap-*`, `col-gap-*`, `row-gap-*`,
  `h-min`, `h-max` и `isolation-auto` с совместимыми именами SF5.
- Полное распознавание существующих gradient, stripe, ring, decoration,
  sizing, position, scroll, grid и stroke-вариантов через Loader.

### Исправлено

- Отрицательные responsive- и state-модификаторы собираются в порядке
  `md:-m-1`, `md:-top-a0`, `hover:-translate-x-1`.
- Loader сопоставляет целый класс и не загружает модуль по подстроке чужого
  имени; классы `gap-*` и `col-gap-*` работают первыми и единственными.
- Сохранено обнаружение совместимых компонентов `file-preview` и `link`.

### Совместимость

- `content-main-*`, `g-*`, `gap-x-*`, `gap-y-*`, `g-x-*`, `g-y-*`,
  `h-content-min` и `h-content-max` продолжают работать в линейке SF5.
- Общий неоднозначный класс `auto` удалён; используйте `isolation-auto`.
...
- Иконка уведомления `success` снова использует семантический цвет
  `--sf-success` и остаётся видимой во всех вариантах поверхности Alerts.

### Совместимость

- Разметка Alerts, публичные классы, Loader API и пути ресурсов не изменены.
- Совместимая Smart-поставка — `simai/ui-smart` `v5.4.1`.
