...
/* harmony import */ var _core_js_position_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__("2e9112dbdda9");

 // Prefer the helper Core published; an isolated bundle uses its own copy.

const positioning = () => globalThis.SF?.Position || _core_js_position_js__WEBPACK_IMPORTED_MODULE_1__["default"];

const ARROW_CLASS = /(?:^|\s)sf-tooltip--arrow-([a-z-]+)/; // The arrow names the tooltip side: a bottom arrow points down at the trigger,
// so the tooltip sits above it. Flipping swaps to the mirrored arrow.

const ARROW_FLIP = {
  'bottom-center': 'top-center',
  'top-center': 'bottom-center',
  'bottom-inline-start': 'top-inline-start',
  'top-inline-start': 'bottom-inline-start',
  'bottom-inline-end': 'top-inline-end',
  'top-inline-end': 'bottom-inline-end',
...
function arrowPlacement(arrow, rtl) {
  const start = rtl ? 'end' : 'start';
  const end = rtl ? 'start' : 'end';
  ...
  switch (arrow) {
    ...
    case 'bottom-inline-start':
      return {
        side: 'block-start',
        align: 'start'
      };

    case 'bottom-inline-end':
      return {
        side: 'block-start',
        align: 'end'
      };
    ...
    // Legacy physical arrows keep their visual side in either direction.
    ...
    case 'left':
      return {
        side: rtl ? 'inline-start' : 'inline-end',
        align: 'center'
      };

    case 'right':
      return {
        side: rtl ? 'inline-end' : 'inline-start',
        align: 'center'
      };
...
class SfTooltipController {
  ...
  } // Shared Framework geometry (SF.Position): the arrow chooses the side, the
  // tooltip flips (with a mirrored arrow) and shifts inside the viewport.


  startPosition() {
    this.stopPosition();
    const reference = this.anchor || this.trigger; // A Smart <sf-tooltip> host has no box of its own (display: contents); the
    // rendered .sf-tooltip inside it is what gets positioned.

    const box = this.surface.localName === 'sf-tooltip' ? this.surface.querySelector('.sf-tooltip') : this.surface;
    if (!box) return;
    const tail = box.matches('[class*="sf-tooltip--arrow-"]') ? box : box.querySelector('[class*="sf-tooltip--arrow-"]');
    const arrow = tail?.className.match(ARROW_CLASS)?.[1] || 'none';
    const rtl = getComputedStyle(reference).direction === 'rtl';
    const {
      side,
      align
    } = arrowPlacement(arrow, rtl);
    const offset = arrow === 'none' ? 'var(--sf-space-1\\/4)' : positioning().resolveLength('var(--sf-tooltip-tail-half)', tail);
    const translate = [box.style.getPropertyValue('translate'), box.style.getPropertyPriority('translate')];
    const tailNode = tail?.querySelector('.sf-tooltip-tail') || null;
    const tailTranslate = tailNode ? [tailNode.style.getPropertyValue('translate'), tailNode.style.getPropertyPriority('translate')] : null; // The static layout centres with translate; anchored coordinates are exact.

    box.style.translate = 'none';
    let shown = arrow;
    const anchor = positioning().anchor(reference, box, {
      side,
      align,
      offset,
      fitHeight: false,
      onPosition: ({
        ...
        if (!tail || arrow === 'none') return;
        const next = actual === side ? arrow : ARROW_FLIP[arrow] || arrow;

        if (next !== shown) {
          tail.classList.replace(`sf-tooltip--arrow-${shown}`, `sf-tooltip--arrow-${next}`);
          shown = next;
        } // A shifted tooltip keeps its tail on the trigger: centred tails move by
        // the shift, limited so they stay clear of the rounded corners.


        if (!tailNode || !/center|^inline-|^left$|^right$/.test(shown)) return;
        const target = reference.getBoundingClientRect();
        const own = box.getBoundingClientRect();
        const vertical = actual === 'block-start' || actual === 'block-end';
        const corner = positioning().resolveLength('var(--sf-tooltip-tail-corner-offset)', tail);
        const limit = Math.max(0, (vertical ? own.width : own.height) / 2 - corner);
        const delta = vertical ? target.left + target.width / 2 - (own.left + own.width / 2) : target.top + target.height / 2 - (own.top + own.height / 2);
        const move = Math.max(-limit, Math.min(limit, delta));
        tailNode.style.translate = vertical ? `${move}px 0` : `0 ${move}px`;
      }
    });
    this.position = {
      anchor,
  ...
  stopPosition() {
    ...
    if (tailNode) {
      if (tailTranslate[0]) tailNode.style.setProperty('translate', tailTranslate[0], tailTranslate[1]);else tailNode.style.removeProperty('translate');
    }

    const shown = this.position.shown();
    if (tail && shown !== arrow) tail.classList.replace(`sf-tooltip--arrow-${shown}`, `sf-tooltip--arrow-${arrow}`);
    if (translate[0]) box.style.setProperty('translate', translate[0], translate[1]);else box.style.removeProperty('translate');
    this.position = null;
  }
