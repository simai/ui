...
function initLineNumbers(hljs) {
  ...
  function addLineNumbersBlockFor(element, options) {
    ...
    if (lines.length > 1 || options.singleLine) {
      ...
      let inlineSize = 'var(--sf-c2)';

      switch (digits) {
        case 1:
          inlineSize = 'var(--sf-c2)';
          break;

        case 2:
          inlineSize = 'var(--sf-c6)';
          break;

        case 3:
          inlineSize = 'var(--sf-d0)';
          break;
      }

      let widthStyle = `style="inline-size:${inlineSize}"`;
      ...
      if (area) {
        ...
        if (textarea) {
          textarea.style.paddingLeft = `calc(${inlineSize} + var(--sf-space-1) + var(--sf-b0))`;
        }
      }
