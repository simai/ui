...
Приведения типов нет. Значение до 64 KiB в JSON. Маршрут допустим только при
...
портов передаётся в `options.ports` как готовый реестр или
`{manifests, bindings}`; без него используется встроенный. Коды:
...
Доставка: значение проверяется по типу выхода и замораживается, затем получает
